...
- **Authentication**: Auth.js (next-auth v5), Credentials provider, JWT sessions
...
### Required for Authentication (Auth.js)

```env
AUTH_SECRET=            # generate with: npx auth secret
AUTH_DEMO_EMAIL=        # demo Credentials login until a real user store is wired up
AUTH_DEMO_PASSWORD=
```
...
Every route under `/dashboard` requires a signed-in session (enforced in `src/proxy.ts`); `src/hooks/use-nav.ts` currently passes all nav items through unfiltered. Items can optionally carry an `access: PermissionCheck` property (`permission` / `role` / `plan` / `feature`, defined in `src/types/index.ts`) for role-based filtering once a real role model exists on top of Auth.js sessions.

### Authentication Patterns

- **Session check (Server Components / Route Handlers)**: `const session = await auth();` from `@/auth` — `session.user` is `undefined` when signed out.
- **Middleware route protection**: `src/proxy.ts` wraps `auth()` and redirects unauthenticated requests to `/auth/sign-in` for any `/dashboard/*` path.
- **Client-side session**: `useSession()` from `next-auth/react` (available via the `SessionProvider` in `src/components/layout/providers.tsx`).
- **Sign in / sign out**: `signIn('credentials', { email, password, redirect: false })` and `signOut({ callbackUrl: '/auth/sign-in' })` from `next-auth/react` on the client, or `signIn`/`signOut` exported from `@/auth` on the server.
- **Adding real users**: `authorize()` in `src/auth.ts` currently checks `AUTH_DEMO_EMAIL`/`AUTH_DEMO_PASSWORD` — swap it for a database lookup (and add a next-auth adapter) when a user table exists.
...
- `AUTH_SECRET`, `AUTH_DEMO_EMAIL`, `AUTH_DEMO_PASSWORD` (Auth.js)
