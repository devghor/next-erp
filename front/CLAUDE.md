...
- **Auth** — Auth.js (next-auth v5), Credentials provider, JWT sessions; `auth()` from `@/auth` on the server, `useSession()`/`signIn()`/`signOut()` from `next-auth/react` on the client, `/dashboard/*` gated in `src/proxy.ts` — see AGENTS.md § Navigation & Authentication
