...
import { importCustomersMutation } from '../api/mutations';
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.failures.length === 0) {
        toast.success(`Imported ${data.imported} customer(s)`);
      } else {
        toast.warning(
          `Imported ${data.imported} customer(s), ${data.failures.length} row(s) failed`
        );
      }
    },
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <code>state</code>, <code>postal_code</code>, <code>country</code>,{' '}
            <code>tax_number</code> and <code>credit_limit</code> columns.
          </DialogDescription>
        </DialogHeader>
