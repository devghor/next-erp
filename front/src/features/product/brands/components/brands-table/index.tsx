...
import { brandsQueryOptions } from '../../api/queries';
import { useBrandFilters } from '../../hooks/use-brand-filters';
import { columns } from './columns';
import { BrandsTableActionBar } from './action-bar';

export function BrandsTable() {
  const filters = useBrandFilters();

  const { data, isPending, isFetching } = useQuery({
    ...brandsQueryOptions(filters),
  ...
  return (
    <div className='flex flex-1 flex-col space-y-4'>
      ...
      >
        {hasSelection && <BrandsTableActionBar table={table} />}
      </DataTableToolbar>
      {isFetching ? (
        <DataTableSkeleton
