...
interface PurchaseItemsEditorProps {
  ...
  onChange: (items: PurchaseItemFormValues[]) => void;
  productOptions: ProductOption[];
}

function emptyRow(): PurchaseItemFormValues {
  return { product_id: 0, qty: 1, net_unit_cost: 0, discount: 0, tax_rate: 0 };
}

function rowTotal(row: PurchaseItemFormValues): number {
  const subTotal = row.qty * row.net_unit_cost - (row.discount ?? 0);
  const tax = subTotal * ((row.tax_rate ?? 0) / 100);
  return Math.max(0, subTotal + tax);
}

export function PurchaseItemsEditor({ items, onChange, productOptions }: PurchaseItemsEditorProps) {
  const updateRow = (index: number, patch: Partial<PurchaseItemFormValues>) => {
              ...
              <TableRow key={index}>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updateRow(index, {
                        ...
                        net_unit_cost: row.net_unit_cost || selected?.cost || 0
                      });
                    }}
                  >
                ...
                <TableCell>
                  <Input
                    ...
                    onChange={(e) => updateRow(index, { net_unit_cost: Number(e.target.value) })}
                  />
                </TableCell>
                <TableCell>
                  <Input
