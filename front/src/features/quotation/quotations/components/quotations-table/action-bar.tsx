...
import { bulkDeleteQuotationsMutation } from '../../api/mutations';
import { quotationKeys } from '../../api/queries';
import type { Quotation } from '../../api/types';

interface QuotationsTableActionBarProps {
  table: Table<Quotation>;
}

export function QuotationsTableActionBar({ table }: QuotationsTableActionBarProps) {
  const [deleteOpen, setDeleteOpen] = useState(false);
  const selectedRows = table.getFilteredSelectedRowModel().rows;

  const bulkDeleteMutation = useMutation({
    ...bulkDeleteQuotationsMutation,
    onSuccess: () => {
      getQueryClient().invalidateQueries({ queryKey: quotationKeys.all });
      toast.success(`${selectedRows.length} quotation(s) deleted successfully`);
      setDeleteOpen(false);
      table.resetRowSelection();
    },
    onError: () => toast.error('Failed to delete selected quotations')
  });
