...
import { QuotationsTableActionBar } from './action-bar';

export function QuotationsTable() {
  const filters = useQuotationFilters();

  const { data, isPending, isFetching } = useQuery({
    ...quotationsQueryOptions(filters),
  ...
  if (isPending) {
    return <QuotationsTableSkeleton />;
  }
  ...
  return (
    <div className='flex flex-1 flex-col space-y-4'>
      ...
      >
        {hasSelection && <QuotationsTableActionBar table={table} />}
      </DataTableToolbar>
      {isFetching ? (
