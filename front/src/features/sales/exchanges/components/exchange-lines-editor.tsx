...
interface ExchangeLinesEditorProps {
  type: 'new' | 'returned';
  label: string;
  lines: ExchangeLineFormValues[];
  onChange: (lines: ExchangeLineFormValues[]) => void;
  productOptions: ProductOption[];
}

function emptyRow(type: 'new' | 'returned'): ExchangeLineFormValues {
  return { type, product_id: 0, qty: 1, net_unit_price: 0, discount: 0, tax_rate: 0 };
}

export function rowTotal(row: ExchangeLineFormValues): number {
...
export function ExchangeLinesEditor({ type, label, lines, onChange, productOptions }: ExchangeLinesEditorProps) {
  const updateRow = (index: number, patch: Partial<ExchangeLineFormValues>) => {
  ...
  const subTotal = lines.reduce((sum, row) => sum + rowTotal(row), 0);
  ...
  return (
    <Field>
      ...
      <div className='flex items-center justify-between'>
        <Button type='button' variant='secondary' size='sm' onClick={() => onChange([...lines, emptyRow(type)])}>
          <Icons.add className='mr-2 h-4 w-4' /> Add Product
        </Button>
        <p className='text-sm font-semibold'>Sub Total: {subTotal.toFixed(2)}</p>
      </div>
    </Field>
  );
}
