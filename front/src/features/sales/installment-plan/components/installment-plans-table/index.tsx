...
import { installmentPlansQueryOptions } from '../../api/queries';
import { useInstallmentPlanFilters } from '../../hooks/use-installment-plan-filters';
import { columns } from './columns';

export function InstallmentPlansTable() {
  const filters = useInstallmentPlanFilters();

  const { data, isPending, isFetching } = useQuery({
    ...installmentPlansQueryOptions(filters),
  ...
  if (isPending) {
    return <InstallmentPlansTableSkeleton />;
  }
...
export function InstallmentPlansTableSkeleton() {
  return (
