'use client';

import { useMutation } from '@tanstack/react-query';
import { toast } from 'sonner';
import { ExportButton } from '@/components/buttons/export-button';
import { exportTaxesPdf, exportTaxesExcel } from '../api/service';
import { useTaxFilters } from '../hooks/use-tax-filters';

function downloadBlob(blob: Blob, filename: string) {
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = filename;
  link.click();
  URL.revokeObjectURL(url);
}

export function TaxExportButtons() {
  const filters = useTaxFilters();

  const pdfMutation = useMutation({
    mutationFn: () => exportTaxesPdf(filters),
    onSuccess: (blob) => downloadBlob(blob, `taxes-${Date.now()}.pdf`),
    onError: () => toast.error('Failed to export PDF')
  });

  const excelMutation = useMutation({
    mutationFn: () => exportTaxesExcel(filters),
    onSuccess: (blob) => downloadBlob(blob, `taxes-${Date.now()}.xlsx`),
    onError: () => toast.error('Failed to export Excel')
  });

  const isPending = pdfMutation.isPending || excelMutation.isPending;

  return (
    <ExportButton
      onExportPdf={() => pdfMutation.mutate()}
      onExportExcel={() => excelMutation.mutate()}
      isPending={isPending}
    />
  );
}
