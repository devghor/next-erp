...
import { bulkDeleteTaxesMutation } from '../../api/mutations';
import { taxKeys } from '../../api/queries';
import type { Tax } from '../../api/types';
import { Can } from '@/components/can';

interface TaxesTableActionBarProps {
  table: Table<Tax>;
}

export function TaxesTableActionBar({ table }: TaxesTableActionBarProps) {
  const [deleteOpen, setDeleteOpen] = useState(false);
  const selectedRows = table.getFilteredSelectedRowModel().rows;

  const bulkDeleteMutation = useMutation({
    ...bulkDeleteTaxesMutation,
    onSuccess: () => {
      getQueryClient().invalidateQueries({ queryKey: taxKeys.all });
      toast.success(`${selectedRows.length} tax(es) deleted successfully`);
      setDeleteOpen(false);
