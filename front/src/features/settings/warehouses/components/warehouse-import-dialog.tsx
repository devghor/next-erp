...
import { importWarehousesMutation } from '../api/mutations';
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.failures.length === 0) {
        toast.success(`Imported ${data.imported} warehouse(s)`);
      } else {
        toast.warning(
          `Imported ${data.imported} warehouse(s), ${data.failures.length} row(s) failed`
        );
      }
    },
    onError: () => toast.error('Failed to import warehouses')
  });
...
export function WarehouseImportDialogTrigger() {
  const [open, setOpen] = useState(false);

  return (
    <>
      <ImportButton onClick={() => setOpen(true)} />
      <WarehouseImportDialog open={open} onOpenChange={setOpen} />
    </>
  );
}
